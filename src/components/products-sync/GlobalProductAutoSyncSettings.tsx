...
interface GlobalProductAutoSyncSettingsProps {
  isOpen: boolean;
  onClose: () => void;
  syncedCount: number;
}

export default function GlobalProductAutoSyncSettings({ 
  isOpen, 
  onClose, 
  syncedCount 
}: GlobalProductAutoSyncSettingsProps) {
  ...
  async function loadConfig() {
    try {
      setLoading(true);
      const response = await fetch("/api/sync/schedule/products");
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">
              Product Auto-Sync Settings
            </h3>
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
              Automatically sync inventory for {syncedCount} mapped products
            </p>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900/20">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 ml-14">
                Automatically sync inventory from Nhanh to Shopify on schedule
              </p>
            </div>
            ...
            {enabled && (
              ...
              <div className="bg-brand-50 border border-brand-200 rounded-lg p-4 dark:bg-brand-900/20 dark:border-brand-800">
                ...
                <ul className="text-sm text-brand-800 dark:text-brand-400 space-y-1">
                  <li>• Will sync inventory for {syncedCount} mapped products</li>
                  <li>• Timezone: Asia/Ho_Chi_Minh (GMT+7)</li>
                  <li>• Inventory automatically updates from Nhanh.vn to Shopify</li>
                  <li>• View logs in database (product_sync_logs table)</li>
                </ul>
              </div>
            </>
          )}
